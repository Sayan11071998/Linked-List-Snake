...
namespace Food
{
	...
	FoodService::FoodService() : random_engine(random_device()) { current_food_item = nullptr; }

	FoodService::~FoodService() { destroyFood(); }


	void FoodService::initialize() { elasped_duration = spawn_duration; }
	...
	void FoodService::updateElaspedDuration() { elasped_duration += ServiceLocator::getInstance()->getTimeService()->getDeltaTime(); }
